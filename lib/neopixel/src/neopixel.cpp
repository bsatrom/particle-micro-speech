...
#elif (PLATFORM_ID == 12) || (PLATFORM_ID == 13) || (PLATFORM_ID == 14) || (PLATFORM_ID == 23) // Argon (12), Boron (13), Xenon (14), Bsom (23)
#include "nrf.h"
#include "nrf_gpio.h"
...
Adafruit_NeoPixel::Adafruit_NeoPixel(uint16_t n, uint8_t p, uint8_t t) : begun(false), type(t), brightness(0), pixels(NULL), endTime(0)
{
  updateLength(n);
  setPin(p);
}
...
void Adafruit_NeoPixel::show(void)
{
  ...
  {
  case TM1803:
  { // TM1803 = 24us reset pulse
    wait_time = 24L;
  }
  break;
  case SK6812RGBW:
  { // SK6812RGBW = 80us reset pulse
    wait_time = 80L;
  }
  break;
  case TM1829:
  { // TM1829 = 500us reset pulse
    wait_time = 500L;
  }
  break;
  ...
  if (type == WS2812B || type == WS2812B_FAST)
  { // Same as WS2812 & WS2813, 800 KHz bitstream
    while (i)
  ...
  else if (type == SK6812RGBW)
  { // similar to WS2812, 800 KHz bitstream but with RGB+W components
    while (i)
    {                                                                         // While bytes left... (4 bytes = 1 pixel)
  ...
  else if (type == WS2812B2 || type == WS2812B2_FAST)
  {                                                                                        // WS2812B with DWT timer
#if (PLATFORM_ID == 6) || (PLATFORM_ID == 8) || (PLATFORM_ID == 10) || (PLATFORM_ID == 88) // Photon (6), P1 (8), Electron (10) or Redbear Duo (88)
    ...
    uint16_t n, uint8_t r, uint8_t g, uint8_t b)
{
  if (n < numLEDs)
  {
    ...
    break;
    case TM1829:
    { // TM1829 is special RBG order
      if (r == 255)
        r = 254; // 255 on RED channel causes display to be in a special mode.
      *p++ = r;
      *p++ = b;
      *p = g;
    }
...
uint32_t Adafruit_NeoPixel::getPixelColor(uint16_t n) const
{
  ...
  break;
  case SK6812RGBW:
  { // SK6812RGBW is RGBW order, but returns packed WRGB color
